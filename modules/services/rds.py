...
def list_rds_public_snapshots(attacker_session):
    if(my_globals.victim_aws_account_ID):
                ...
                if snapshots:
                    for snapshot in snapshots:
                        ...
                        if snapshot_arn in my_globals.victim_aws_account_ID:
                            print(
                                f"{Fore.MAGENTA} | Region: {region} | Snapshot ID: {snapshot.get('DBClusterSnapshotArn', '')}")
                            found_victim_snapshots += 1
                else:
                    sys.stdout.write("\r\033[K")
                    sys.stdout.write(
                        f"{Fore.LIGHTBLACK_EX} | No public RDS snapshots found in region {region} for account {my_globals.victim_aws_account_ID}")
                    sys.stdout.flush()
